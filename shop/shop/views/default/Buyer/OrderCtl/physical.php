    ...
    <div class="order_content">
        <div class="order_content_title clearfix">
            <form method="get" id="search_form" action="index.php" >
                ...
                <p class="order_types">
                    <a <?php if($status == '' &&  !$recycle):?>class="currect"<?php endif;?> href="<?= Yf_Registry::get('url') ?>?ctl=Buyer_Order&met=physical"><?=__('全部订单')?></a>
                    <a <?php if($status == 'wait_pay'):?>class="currect"<?php endif;?> href="<?= Yf_Registry::get('url') ?>?ctl=Buyer_Order&met=physical&status=wait_pay"><?=__('待付款')?></a>
                    <a <?php if($status == 'order_payed'):?>class="currect"<?php endif;?> href="<?= Yf_Registry::get('url') ?>?ctl=Buyer_Order&met=physical&status=order_payed"><?=__('待发货')?></a>
                    <a <?php if($status == 'wait_confirm_goods'):?>class="currect"<?php endif;?> href="<?= Yf_Registry::get('url') ?>?ctl=Buyer_Order&met=physical&status=wait_confirm_goods"><?=__('待收货')?></a>
                    <a <?php if($status == 'finish'):?>class="currect"<?php endif;?> href="<?= Yf_Registry::get('url') ?>?ctl=Buyer_Order&met=physical&status=finish"><?=__('待评价')?></a>
                </p>
                ...
                <p class="order_types serc_p">
                    <a <?php if($recycle):?>class="currect"<?php endif;?> href="<?= Yf_Registry::get('url') ?>?ctl=Buyer_Order&met=physical&recycle=1"><i class="iconfont icon-lajitong icon_size20"></i><?=__('订单回收站')?></a>
                </p>
                    ...
                    <tr class="tr_title">
                        <th colspan="8" class="order_mess clearfix">
                            <p class="order_mess_one">
                                ...
                                <a target="_blank" href="<?= Yf_Registry::get('url') ?>?ctl=Shop&met=index&id=<?=($val['shop_id'])?>"><i class="iconfont icon-icoshop"></i><?=($val['shop_names'])?></a>
                            </p>
                        </th>
                    </tr>
                                        ...
                                        <td class="td_color widt4">
                                            ...
                                            ?>
                                                ...
                                                if((($val['order_status'] == Order_StateModel::ORDER_PAYED && $ogval['goods_return_status'] == Order_StateModel::ORDER_GOODS_RETURN_NO) || ($val['order_status'] == Order_StateModel::ORDER_FINISH  && $ogval['goods_refund_status'] == Order_StateModel::ORDER_GOODS_RETURN_NO))&& !$val['order_source_id'] && $val['order_refund_status'] == Order_StateModel::ORDER_REFUND_NO && $ogval['order_goods_num'] > $ogval['order_goods_returnnum'] && $ogval['goods_price'] > 0
                                                    ...
                                                    ?>
                                                        <p>
                                                        <a target="_blank" href="<?= Yf_Registry::get('url') ?>?ctl=Buyer_Service_Return&met=index&act=add&oid=<?=($val['order_id'])?>&gid=<?=($ogval['order_goods_id'])?>" class="to_views"><?=__('退款/退货')?></a>
                                                        </p>
                                                    <?php } } ?>
                            ...
                            <?php if(!$val['order_source_id']){?> <!--分销商不能操作SP订单  -->
                                ...
                                <?php if($val['order_status'] == Order_StateModel::ORDER_WAIT_PAY):?>
                                    <p class="rest">
                                        ...
                                        <span class="fnTimeCountDown" data-end="<?=$val['cancel_time']?>">
                                            ...
                                            <span class="hour">00</span><span><?=__('时自动关闭')?></span>
                                        </span>
                                    </p>
                                    </span>
                                ...
                                <?php if($val['order_refund_status'] !== Order_StateModel::ORDER_REFUND_IN && $val['order_return_status'] !== Order_StateModel::ORDER_GOODS_RETURN_IN  &&  $val['order_status'] == Order_StateModel::ORDER_WAIT_CONFIRM_GOODS ): ?>
                                    <p class="rest">
                                        ...
                                        <span class="fnTimeCountDown" data-end="<?=$val['order_receiver_date']?>">
                                            <span><?=__("剩余")?></span>
                                            <span class="day" >00</span><span><?=__('天')?></span>
                                            <span class="hour">00</span><span><?=__('时自动收货')?></span>
                                        </span>
                                    </p>
                                    <p> <a onclick="confirmOrder('<?=$val['order_id']?>')" class="to_views "><i class="iconfont icon-duigou1 icon_size22"></i><?=__('确认收货')?></a></p>
                                <?php endif;?>
                                ...
                                <?php if($val['order_status'] == Order_StateModel::ORDER_FINISH ): ?>
                                    ...
                                    <?php if($val['order_buyer_evaluation_status'] == 1): ?>
                                        <p><a target="_blank" href="<?= Yf_Registry::get('url') ?>?ctl=Buyer_Order&met=evaluation" class="to_views"><i class="iconfont icon-woyaopingjia icon_size22"></i>
