...
class Differ
{
    ...
    private function diffPrimaryKey(ChangedTable $changedTable)
    {
		...
		$to_k = $toTable->getPrimaryKeys();
		$from_k = $fromTable->getPrimaryKeys();
		
        if (empty($to_k) && !empty($from_k)) {
    ...
    {		$migrationScript = '';
		...
		$changeScript = '';
		if ($databaseDiff->getChangedTables())
		{
			foreach ($databaseDiff->getChangedTables() as $changedTable) {
				if ($displayProgress) {
					$changeScript .= sprintf("SELECT 'Altering table %s';" . PHP_EOL, $changedTable->getName());
				}
			
				if ($cc = $changedTable->generateAlterScript())
				{
					$changeScript .= PHP_EOL . sprintf('-- changed table `%s`' . PHP_EOL . PHP_EOL, $changedTable->getName());
				
					$changeScript .= $cc . PHP_EOL;
				}
			}
		}
	
		if ($changeScript || $databaseDiff->getNewTables())
		{
