        ...
        if (!isset(self::$cache[$name]))
        {
            if (2 == $config_cache[$name]['cacheType'])
            {
                ...
                elseif ('data' == $name)
                {
                    ...
                    {
                        self::$cache['_memcache_data'] = Yf_Cache_Memcache::getInstance($name);
                    }
            ...
            else
            {
				...
				{
					self::$cache[$name] = new Yf_Cache_Apcu($config_cache[$name]);
				}
				elseif (extension_loaded('apc') && function_exists('apc_fetch'))
				{
					self::$cache[$name] = new Yf_Cache_Apc($config_cache[$name]);
				}
				else
				{
					$key = '_file_' . $name;
