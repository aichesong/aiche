...
(function() {
    ...
    function show_msg_list(u_id){
        var ajax_chatlog_url = ImApiUrl+"?ctl=Api_Chatlog&met=get&u="+getCookie('user_account')+"&to="+u_id+"&day=1&typ=json&is_wap=1";   
            $.get(ajax_chatlog_url,function(json){ 
                 ...
                 goto_top();
                 $('.chat_header_icon_from').attr('src',receiver_logo);
    ...
    window.IM = window.IM || {
       ...
       EV_login : function(user_account, sig, timestamp) {
        ...
        HTML_pushMsg_addHTML : function(msgtype, you_sender, version,
            ...
            {
                //receiver_logo = "";
                $.post(ImApiUrl+"?ctl=UserApi&met=getUserInfosByName&typ=json",{"user_name":getQueryString('contact_you'),"k":k,"u":u} ,function(data) {
                    if(data.status == 200)
                    {
                        console.info(data);
