...
class Plugin_Xhprof implements Yf_Plugin_Interface
{
	...
	function endXhprof($data = null)
	{
		if ($this->xhprof_on)
		{
            ...
            $file_name = sprintf('%s-%s', $ctl, $met);
            
            $str =  sprintf('<a href="%s" target="_blank">%ss</a>', Yf_Registry::get('base_url') . '/shop/controllers/Plugin/Xhprof/xhprof_html/index.php?run=' . $run_id . '&source=' . $ctl . '_' . $met, $file_name) . "<br />";
            
            
            file_put_contents(LAN_PATH . '/' . $file_name, $str);
		}
	}
}
